// BOJ 20150 [Line Segment Intersection 4]
...
impl Line {
    fn intersect(&self, l: &Line) -> bool {
        let ccw1 = ccw(&self.p1, &self.p2, &l.p1) * ccw(&self.p1, &self.p2, &l.p2);
        let ccw2 = ccw(&l.p1, &l.p2, &self.p1) * ccw(&l.p1, &l.p2, &self.p2);
        if ccw1 == 0 && ccw2 == 0 {
            if self.p1.x == self.p2.x {
                (self.p1.y.min(self.p2.y) <= l.p1.y && l.p1.y <= self.p1.y.max(self.p2.y)) ||
                (self.p1.y.min(self.p2.y) <= l.p2.y && l.p2.y <= self.p1.y.max(self.p2.y))
            } else {
                (self.p1.x.min(self.p2.x) <= l.p1.x && l.p1.x <= self.p1.x.max(self.p2.x)) ||
                (self.p1.x.min(self.p2.x) <= l.p2.x && l.p2.x <= self.p1.x.max(self.p2.x))
            }
        } else {
            ccw1 <= 0 && ccw2 <= 0
        }
...
impl Ord for Point {
    fn cmp(&self, other: &Self) -> Ordering {
        self.x.cmp(&other.x).then(self.e.cmp(&other.e)).then(self.y.cmp(&other.y))
    }
}
...
pub fn main() {
    ...
    for i in 0..next(&mut it) {
        ...
        points.push(Point { x: x1, y: y1, e: e1, i });
        points.push(Point { x: x2, y: y2, e: e2, i });
    }
    ...
    if yp {
        ...
        for pair in points.chunks(2) {
            ...
            new_points.push(Point { x: x1, y: y1, e: e1, i: pair[0].i });
            new_points.push(Point { x: x2, y: y2, e: e2, i: pair[1].i });
        }
        points = new_points;
    }
    ...
    for point in points {
        ...
        if point.e {
            ...
            if let Some(p) = set.range(lines[point.i].clone()..).skip(1).next() {
                if p.intersect(&lines[point.i]) {
                    writeln!(so, "1").unwrap();
                    return;
                }
