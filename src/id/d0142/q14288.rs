...
pub fn main() { read();
    ...
    let mut ft1 = FenwickTree::new(n+1);
    let mut ft2 = FenwickTree::new(n+1);
    let mut flag = true;
    ...
    for _ in 0..m {
        ...
        if q == 3 { flag = !flag; }
            ...
            if flag {  ft1.range_update(id[i], id[i] + sz[i] - 1, w); }
            else { ft2.point_update(id[i], w); }
        } else {
            let i = next::<usize>();
            println!("{}", ft1.point_query(id[i]) + ft2.range_query(id[i], id[i] + sz[i] - 1));
        }
    }
}
