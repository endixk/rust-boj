// BOJ 13545 [Sequence and Queries 0]
...
pub fn main() { read();
    ...
    assert!(sq * sq >= n);
    let mut arr = vec![100000; n];
    let (mut x, mut y) = (100000, 100000);
    for i in 1..n {
        match next::<i8>() {
            1 => { x += 1; arr[i] = x; }
            _ => { x -= 1; arr[i] = x; if x < y { y = x; } }
        }
    }
    let mut k = 0;
    for i in 0..n { arr[i] -= y; k = k.max(arr[i] as usize); }

    let m = next::<usize>();
    let mut qv = (0..m).map(|i|
        (next::<usize>() - 1, next::<usize>(), i)
    ).collect::<Vec<_>>();
